function ForEachList(options) {
  ...
  contents += '#define ' + self.name + (options.maxIterations - 1) + '_0(MACRO,ARGS...) MACRO(0,' + options.isEmpty.name + '(' + options.post.name + '(ARGS)),' + (options.maxIterations - 1) + ',' + options.list.name + '(' + options.pre.name + '(ARGS)),\n';
